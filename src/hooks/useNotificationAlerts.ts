...
// Pleasant sounds for patients
const PLEASANT_CHIME_URL = 'https://actions.google.com/sounds/v1/alarms/gentle_alarm.ogg';
const MEDICATION_SOUND_URL = 'https://actions.google.com/sounds/v1/alarms/medium_bell_ringing_near.ogg';
// Alert sound for emergencies
const EMERGENCY_SOUND_URL = 'https://actions.google.com/sounds/v1/alarms/alarm_clock.ogg';
...
export const useNotificationAlerts = () => {
  ...
  const checkMedications = useCallback(async () => {
    ...
    const { data: meds } = await supabase
      ...
      .eq('patient_id', patientRecord.id).eq('active', true);
    ...
    for (const med of meds) {
      ...
      for (const time of med.times_array) {
        const medTime = time.substring(0, 5);
        ...
        if (Math.abs(curMinutes - medMinutes) <= 1) {
          ...
          playSound(MEDICATION_SOUND_URL);
          showBrowserNotification('💊 Medication Reminder', `Time to take ${med.name} (${med.dosage})`, `med-${key}`);
          toast.info(`💊 Time to take ${med.name} (${med.dosage})`, { duration: 10000 });
        }
      }
    }
  ...
  const checkAppointments = useCallback(async () => {
    ...
    const { data: appts } = await supabase
      ...
      .eq('patient_id', patientRecord.id).eq('date', today);
    ...
    for (const appt of appts) {
      ...
      const diff = (apptH * 60 + apptM) - curMinutes;

      if (diff >= 0 && diff <= 15) {
        checkedApptsRef.current.add(key);
        playSound(PLEASANT_CHIME_URL);
        showBrowserNotification('📅 Appointment Reminder',
          `${appt.title} at ${appt.time}${appt.doctor ? ` with ${appt.doctor}` : ''}`, `appt-${key}`);
        toast.info(`📅 ${appt.title} ${diff === 0 ? 'is now!' : `in ${diff} minutes`}`, { duration: 10000 });
      }
    }
  }, [patientRecord, profile]);
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel('emergency-alerts')
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'emergency_alerts' },
        (payload: any) => {
          playSound(EMERGENCY_SOUND_URL);
          showBrowserNotification('🚨 EMERGENCY ALERT', 'A patient has triggered an emergency alert!', `emergency-${payload.new.id}`);
          toast.error('🚨 EMERGENCY ALERT from a patient!', { duration: 30000, description: 'Check notifications for details.' });
        })
      .subscribe();
    return () => { supabase.removeChannel(channel); };
  }, [user, profile]);

  // Periodic checks for patients
  useEffect(() => {
    if (profile?.role !== 'patient') return;
    checkMedications();
    checkAppointments();
    const interval = setInterval(() => { checkMedications(); checkAppointments(); }, 30000);
